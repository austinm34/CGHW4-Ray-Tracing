...
HitRecord Sphere::intersect(const Ray &ray) {
    ...
    result.position = ray.origin + result.t * ray.direction;	// r = p + td   so this is the location of where the hit occured.
    result.normal = (result.position - c).normalized();			// this is the surface normal. it'll be used in calculating how strong the reflection is.
